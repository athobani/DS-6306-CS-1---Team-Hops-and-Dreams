...
plot_usmap(data=medians, values = "MedianABV", color = "black") + labs(title = "IBU by State") + scale_fill_continuous(low = "white", high = "navy", name = "MedianABV")
```
...
train = beerbrew1[trainIndices,]
test = beerbrew1[-trainIndices,]
classifying <- knn(train[,c(7,8)],test[,c(7,8)],train$type, prob = TRUE, k = 5)  
...
for(j in 1:iterations)
{
  ...
  model = naiveBayes(train[,c(7,8)],(train$type))
  table(predict(model,test[,c(7,8)]),(test$type))
